...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_mysanctum_team !== undefined) searchParams.set('is_mysanctum_team', params.is_mysanctum_team.toString());
...
export function useMySanctumTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'mysanctum-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view MySanctum templates');
      }

      const response = await fetch(`${API_URL}/templates/mysanctum-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
