...
const getDefaultModel = (accessibleModels: ModelOption[]): string => {
  ...
  // mysanctum/basic should be first for free users since power is not accessible
  const basicModel = accessibleModels.find(m => m.id === 'mysanctum/basic');
  if (basicModel) return basicModel.id;

  const powerModel = accessibleModels.find(m => m.id === 'mysanctum/power');
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      const powerModel = availableModels.find(m => m.id === 'mysanctum/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to mysanctum/power');
        setSelectedModel('mysanctum/power');
      }
    }
