...
import { ToolCallInput } from '@/components/thread/mysanctum-computer';
...
import { useMySanctumComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/mysanctum-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useMySanctumComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure MySanctumComputer switches to tools view
