...
const MOBILE_STORAGE_KEY = 'mysanctum-mobile-banner-dismissed';
const DESKTOP_STORAGE_KEY = 'mysanctum-desktop-banner-dismissed';

const STORE_LINKS = {
  ios: 'https://apps.apple.com/ie/app/mysanctum/id6754448524',
  android: 'https://play.google.com/store/apps/details?id=com.mysanctum.app',
};

const DOWNLOAD_LINKS = {
  windows: 'https://download.mysanctum.com/desktop/latest/windows/MySanctum%20Setup%201.0.0.exe',
  macArm: 'https://download.mysanctum.com/desktop/latest/macos/MySanctum-1.0.0-arm64.dmg',
  macIntel: 'https://download.mysanctum.com/desktop/latest/macos/MySanctum-1.0.0-x64.dmg',
};

type DesktopPlatform = 'windows' | 'mac';

type MySanctumAppBannersProps = {
  /**
...
// MySanctum symbol SVG (inline to avoid loading issues)
function MySanctumSymbol({ size = 24, className }: { size?: number; className?: string }) {
  return (
    <svg 
      width={size} 
      height={size} 
...
export function MySanctumAppBanners(props: MySanctumAppBannersProps) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('[MySanctumAppBanners] flags', { disableMobileAdvertising });
    }
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 rounded-xl bg-foreground dark:bg-white flex items-center justify-center shadow-sm">
                  <MySanctumSymbol size={20} className="text-background dark:text-black" />
                </div>
              </div>
            </motion.div>
          ) : (isExpanded || !showBothBanners) ? (
              ...
              {desktopVisible && (
                ...
                >
                  <div className="relative bg-white dark:bg-[#2a2a2a] rounded-xl shadow-xl overflow-hidden border border-border/60 dark:border-[#232324]">
                    ...
                    <div className="relative h-[80px] bg-muted dark:bg-[#e8e4df] flex items-center justify-center">
                      <div className="w-[160px] h-[50px] bg-background dark:bg-white rounded-lg p-2 relative flex items-center justify-center border border-border/40 dark:border-transparent">
                        ...
                        <div className="w-8 h-8 bg-foreground dark:bg-[#1a1a1a] rounded-lg flex items-center justify-center">
                          <MySanctumSymbol size={16} className="text-background dark:text-white" />
                        </div>
                    ...
                    <div className="p-4 bg-muted/50 dark:bg-[#161618]">
                      <h3 className="text-foreground dark:text-white text-sm font-semibold mb-1">
                        MySanctum for Desktop is here
                      </h3>
                      <p className="text-muted-foreground dark:text-white/60 text-xs leading-relaxed mb-3">
                        Hand it off to MySanctum. From anywhere on your {desktopPlatform === 'mac' ? 'Mac' : 'Desktop'}. Download now.
                      </p>
