...
const ModeLogo = memo(function ModeLogo({ 
  ...
  return (
    <span className="flex-shrink-0 relative" style={{ height: `${height}px`, width: 'auto' }}>
      ...
      <img
        src={lightSrc}
        alt={mode === 'advanced' ? 'MySanctum Advanced' : 'MySanctum Basic'}
...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'mysanctum/basic' || m.label === 'MySanctum Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'mysanctum/power' || m.label === 'MySanctum Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'mysanctum/basic' && m.id !== 'mysanctum/power' && 
             m.label !== 'MySanctum Basic' && m.label !== 'MySanctum Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access MySanctum Advanced mode',
        });
      }
    }
