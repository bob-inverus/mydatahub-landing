...
import { useMySanctumComputerStore, useSetIsSidePanelOpen } from '@/stores/mysanctum-computer-store';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const openFileInComputer = useMySanctumComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = useMySanctumComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = useMySanctumComputerStore((state) => state.setSandboxContext);
  ...
  // Sync MySanctum Computer sandbox context with current sandbox
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show MySanctumComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show MySanctumComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | MySanctum`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive Worker conversation powered by MySanctum`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | MySanctum`);
      }
