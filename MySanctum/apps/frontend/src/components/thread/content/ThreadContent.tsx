...
// Reusable agent header - shows MySanctum logo for MySanctum, avatar+name for others
const AgentHeader = memo(function AgentHeader({ agentInfo }: { agentInfo: AgentInfo }) {
  if (agentInfo.name === "MySanctum") {
    return (
      <img
        src="/mysanctum-logomark-white.svg"
        alt="MySanctum"
...
export const ThreadContent: React.FC<ThreadContentProps> = memo(
  function ThreadContent({
    ...
    isPreviewMode = false,
    agentName = "MySanctum",
    agentAvatar = <MySanctumIcon size={14} />,
    emptyStateComponent,
    threadMetadata,
    scrollContainerRef,
    threadId,
    ...
    const agentInfo = useMemo<AgentInfo>(() => {
      ...
      if (recentAssistantWithAgent?.agents?.name) {
        ...
        const name =
          typeof rawName === "string" ? rawName : String(rawName || "MySanctum");
        return {
          name,
          avatar: (
            <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
              <MySanctumIcon size={14} />
            </div>
          ),
        };
      }
      const fallbackName = typeof agentName === "string" ? agentName : "MySanctum";
      return {
        name: fallbackName || "MySanctum",
        avatar: agentAvatar,
      };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="space-y-2">
                  <div className="animate-shimmer inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium text-primary bg-primary/10 rounded-md border border-primary/20">
                    ...
                    <MySanctumLoader size="small" className="ml-auto" />
                  </div>
                </div>
              </div>
            </div>
