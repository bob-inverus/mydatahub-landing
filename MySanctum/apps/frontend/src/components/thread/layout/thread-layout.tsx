...
import { MySanctumComputer, ToolCallInput } from '@/components/thread/mysanctum-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // MySanctum Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useMySanctumComputerStore();
  ...
  // Handle file click - now opens in MySanctum Computer instead of modal
  ...
  const selectedFilePath = useMySanctumComputerStore((state) => state.selectedFilePath);
