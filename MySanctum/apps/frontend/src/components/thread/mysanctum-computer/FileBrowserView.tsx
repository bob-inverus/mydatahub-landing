...
import { useMySanctumComputerStore } from '@/stores/mysanctum-computer-store';
import { useFileViewerStore } from '@/stores/file-viewer-store';
import { usePresentationViewerStore } from '@/stores/presentation-viewer-store';
import { Badge } from '@/components/ui/badge';
import { VersionBanner } from './VersionBanner';
import { MySanctumComputerHeader } from './MySanctumComputerHeader';
...
interface FileBrowserViewProps {
  ...
  projectId?: string;
  /** 
   * 'default' - Side panel view in MySanctum Computer (shows all files)
      ...
      if (file.is_dir) {
        ...
        if (isPresentationFolder(file)) {
          ...
          if (isLibraryView && project?.sandbox?.sandbox_url) {
            ...
            // In side panel view, use mysanctum computer store
            openFile(file.path);
          }
        } else {
      ...
      <div className="flex flex-col min-h-screen bg-background">
        ...
        <Dialog open={revertModalOpen} onOpenChange={setRevertModalOpen}>
          <DialogContent className="sm:max-w-md rounded-xl bg-background border border-border">
            ...
            {revertLoadingInfo ? (
              <div className="py-6 flex items-center justify-center"><MySanctumLoader size="medium" /></div>
            ) : revertCommitInfo ? (
              <div className="mt-2">
            ...
            <DialogFooter>
              ...
              <Button onClick={performRevert} disabled={revertInProgress}>
                {revertInProgress ? (<><MySanctumLoader size="small" className="mr-2" />Restoring...</>) : 'Restore'}
              </Button>
            </DialogFooter>
            <DialogClose />
          </DialogContent>
