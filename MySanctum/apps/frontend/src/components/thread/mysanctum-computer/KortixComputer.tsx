...
import { 
  useMySanctumComputerStore,
  useMySanctumComputerPendingToolNavIndex,
  useMySanctumComputerClearPendingToolNav,
} from '@/stores/mysanctum-computer-store';
...
interface MySanctumComputerProps {
  isOpen: boolean;
  onClose: () => void;
...
const FLOATING_LAYOUT_ID = 'mysanctum-computer-float';

export const MySanctumComputer = memo(function MySanctumComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useMySanctumComputerPendingToolNavIndex();
  const clearPendingToolNav = useMySanctumComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[MySanctumComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the MySanctum Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the MySanctum Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="mysanctum-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
