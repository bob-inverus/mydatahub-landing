...
// MySanctum agent modes switcher - isolated from typing state
        ...
        <TooltipTrigger asChild>
          ...
          >
            {((loading || isUploading) && !isAgentRunning) ? (
              <MySanctumLoader size="small" customSize={20} variant={buttonLoaderVariant} />
            ) : showAddToQueue ? (
...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const [isModeDismissing, setIsModeDismissing] = useState(false);    // MySanctum Agent Modes feature flag
    ...
    // Check if selected agent is MySanctum based on agent data
    // While loading, default to MySanctum (assume MySanctum is the default agent)
    const selectedAgent = agents.find(agent => agent.agent_id === selectedAgentId);
    const sunaAgent = agents.find(agent => agent.metadata?.is_suna_default === true);
    const isSunaAgent = isLoadingAgents 
        ? true // Show MySanctum modes while loading
