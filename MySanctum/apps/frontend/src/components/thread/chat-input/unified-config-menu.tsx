...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || sunaAgent)) {
            return <AgentAvatar isSunaDefault={true} agentName="MySanctum" size={size} className="flex-shrink-0 !border-0" />;
        }
                    ...
                    {canLoadMore && (
                        <div className={cn("pt-2", compact ? "px-2" : "px-3")}>
                            ...
                            >
                                {isFetching ? (
                                    <>
                                        <MySanctumLoader size="small" className="mr-2" />
                                        Loading...
                                    </>
                                ) : (
                                    `Load more`
    ...
    // Check if current agent is MySanctum (used in multiple places)
    const isMySanctumAgent = !displayAgent?.name || displayAgent?.name === 'MySanctum';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                ...
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isMySanctumAgent ? 'MySanctum' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderSunaAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isMySanctumAgent]);
        ...
        >
            {onAgentSelect ? (
                <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                    {isMySanctumAgent ? (
                        <MySanctumIcon variant="logomark" size={14} />
                    ) : (
                        <>
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isMySanctumAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                ...
                <span className="inline-block">
                    ...
                    >
                        <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                            ...
                            <span className="truncate text-sm font-medium">MySanctum</span>
                            <ChevronDown size={12} className="opacity-60 flex-shrink-0" />
                        </div>
                    </Button>
                </span>
