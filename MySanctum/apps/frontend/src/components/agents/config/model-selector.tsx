...
// Helper to render model labels with special styling for MySanctum modes
const ModelLabel = ({ label, className }: { label: string; className?: string }) => {
    if (label === 'MySanctum Advanced Mode') {
        return (
            <span className={cn("flex items-center gap-2", className)}>
                <span className="font-medium">MySanctum</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/10 dark:bg-primary/15 rounded-full">
                    <MySanctumIcon size={12} variant="symbol" />
...
export function AgentModelSelector({
  ...
  const handleSelect = (modelId: string) => {
    ...
    if (hasAccess) {
      ...
      const isPowerModel = modelId === 'mysanctum/power';
      openPricingModal({
        isAlert: true,
        alertTitle: isPowerModel ? 'Upgrade to access MySanctum Advanced mode' : 'Upgrade to access this model',
      });
    }
  };
  ...
  const renderModelOption = (model: any, index: number) => {
    ...
    const isPowerModel = model.id === 'mysanctum/power';
        ...
        <TooltipTrigger asChild>
          ...
          {!accessible && !isLocalMode() ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{isPowerModel ? 'Upgrade to access MySanctum Advanced mode' : 'Upgrade to access this model'}</p>
            </TooltipContent>
          ) : isLowQuality ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
