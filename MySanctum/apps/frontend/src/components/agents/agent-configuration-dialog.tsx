    ...
    if (!isNameEditable) {
      if (isSunaAgent) {
        toast.error("Name cannot be edited", {
          description: "MySanctum's name is managed centrally and cannot be changed.",
        });
      }
      setEditName(formData.name);
      setIsEditingName(false);
    ...
    if (!isSystemPromptEditable) {
      if (isSunaAgent) {
        toast.error("System prompt cannot be edited", {
          description: "MySanctum's system prompt is managed centrally.",
        });
      }
      return;
    }
    ...
    if (!areToolsEditable) {
      if (isSunaAgent) {
        toast.error("Tools cannot be edited", {
          description: "MySanctum's tools are managed centrally.",
        });
      }
      return;
    }
        ...
        <DialogContent className="max-w-5xl h-[85vh] overflow-hidden p-0 gap-0 flex flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {exportMutation.isPending ? (
                    <MySanctumIconAnimated size={16} speed={1.5} />
                  ) : (
          ...
          {isLoading ? (
            <div className="flex-1 flex items-center justify-center">
              <MySanctumIconAnimated size={80} speed={1} />
            </div>
          ) : (
                    ...
                    {isSunaAgent && (
                      <Alert className="mb-4 bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-900">
                        ...
                        <AlertDescription className="text-sm text-blue-800 dark:text-blue-300">
                          You can't edit the main MySanctum, but you can create a new AI Worker that you can modify as you wish.
                        </AlertDescription>
                      </Alert>
                    )}
          ...
          <DialogFooter className="px-6 py-4 border-t bg-background flex-shrink-0">
            ...
            >
              {isSaving ? (
                <>
                  <MySanctumIconAnimated size={16} speed={1.5} className="mr-1" />
                  Saving...
                </>
              ) : (
                <>
