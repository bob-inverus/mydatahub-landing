...
import { useMySanctumTeamTemplates } from '@/hooks/secure-mcp/use-secure-mcp';
...
export function AgentCreationModal({ open, onOpenChange, onSuccess }: AgentCreationModalProps) {
  ...
  const { data: templates } = useMySanctumTeamTemplates({ enabled: open });
  ...
  const handleCardClick = (template: any) => {
    const marketplaceTemplate: MarketplaceTemplate = {
      ...
      is_mysanctum_team: template.is_mysanctum_team || false,
            ...
            <div className="p-5 sm:p-8">
              ...
              <div className="flex flex-col items-center text-center mb-6 sm:mb-8">
                <div className="mb-3 sm:mb-4 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-muted/50">
                  <MySanctumIcon size={28} variant="symbol" className="sm:hidden" />
                  <MySanctumIcon size={36} variant="symbol" className="hidden sm:block" />
                </div>
