...
interface MySanctumComputerState {
  // === SANDBOX CONTEXT ===
  ...
  // Clear pending tool nav after MySanctumComputer processes it
...
export const useMySanctumComputerStore = create<MySanctumComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[MySanctumComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
          set({
      ...
      clearFileState: () => {
        console.log('[MySanctumComputerStore] Clearing file state');
        set({
          ...initialFileState,
        });
      },
      ...
      reset: () => {
        console.log('[MySanctumComputerStore] Full reset');
        set(initialState);
      },
    }),
    {
      name: 'mysanctum-computer-store',
    }
  )
);
...
// Sandbox context
export const useMySanctumComputerSandboxId = () =>
  useMySanctumComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useMySanctumComputerStore((state) => state.setSandboxContext);

// Main view state
export const useMySanctumComputerActiveView = () => 
  useMySanctumComputerStore((state) => state.activeView);

// Individual selectors for files state (stable, primitive values)
export const useMySanctumComputerFilesSubView = () =>
  useMySanctumComputerStore((state) => state.filesSubView);

export const useMySanctumComputerCurrentPath = () =>
  useMySanctumComputerStore((state) => state.currentPath);

export const useMySanctumComputerSelectedFilePath = () =>
  useMySanctumComputerStore((state) => state.selectedFilePath);

export const useMySanctumComputerFilePathList = () =>
  useMySanctumComputerStore((state) => state.filePathList);

export const useMySanctumComputerCurrentFileIndex = () =>
  useMySanctumComputerStore((state) => state.currentFileIndex);

// Legacy combined selector (for backward compatibility) - use individual selectors in components
export const useMySanctumComputerFilesState = () => ({
  filesSubView: useMySanctumComputerStore((state) => state.filesSubView),
  currentPath: useMySanctumComputerStore((state) => state.currentPath),
  selectedFilePath: useMySanctumComputerStore((state) => state.selectedFilePath),
  filePathList: useMySanctumComputerStore((state) => state.filePathList),
  currentFileIndex: useMySanctumComputerStore((state) => state.currentFileIndex),
});

// Actions are stable references (functions don't change)
export const useMySanctumComputerActions = () =>
  useMySanctumComputerStore((state) => ({
...
export const useMySanctumComputerPendingToolNavIndex = () =>
  useMySanctumComputerStore((state) => state.pendingToolNavIndex);

export const useMySanctumComputerClearPendingToolNav = () =>
  useMySanctumComputerStore((state) => state.clearPendingToolNav);

// Side panel state selectors
export const useIsSidePanelOpen = () =>
  useMySanctumComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useMySanctumComputerStore((state) => state.setIsSidePanelOpen);
