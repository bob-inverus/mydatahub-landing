    ...
    try {
      ...
      if (data.user) {
        ...
        if (vaultError && vaultError.code !== 'PGRST116') {
          // Error other than "not found" - redirect to setting-up to initialize
          console.log('⚠️ Error checking vault - redirecting to setting-up');
          finalDestination = '/setting-up'
        } else if (!vault || !vault.is_initialized) {
          // Vault doesn't exist or not initialized - go to setting-up
          console.log('⚠️ Vault not initialized - redirecting to setting-up');
          finalDestination = '/setting-up'
        } else {
          // Vault exists and is initialized - check if they need to select a plan
          console.log('✅ Vault already initialized - redirecting to activate-trial');
          finalDestination = '/activate-trial'
        }
      }
