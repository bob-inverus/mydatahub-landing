...
export async function signIn(prevState: any, formData: FormData) {
  ...
  // For desktop app, use custom protocol (mysanctum://auth/callback) - same as mobile
  // For web, use standard origin (https://mysanctum.com/auth/callback)
  let emailRedirectTo: string;
  if (isDesktopApp && origin.startsWith('mysanctum://')) {
    // Match mobile implementation - simple protocol URL with optional terms_accepted
    const params = new URLSearchParams();
    if (acceptedTerms) {
      params.set('terms_accepted', 'true');
    }
    emailRedirectTo = `mysanctum://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
  } else {
