...
const LinkedInIcon = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className="text-blue-600 dark:text-blue-400">
...
export default function TeamPage() {
  ...
  // Close expanded card when clicking outside on mobile
  useEffect(() => {
    if (!isMobile || !expandedCard) return

    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as Element
      if (!target.closest('.team-member-card')) {
  ...
  const handleCardClick = (personId: string, linkedinUrl: string) => {
    if (isMobile) {
      // On mobile, toggle the expanded state first
      if (expandedCard === personId) {
        // If already expanded, navigate to LinkedIn
        window.open(linkedinUrl, '_blank', 'noopener,noreferrer')
      } else {
        // If not expanded, expand the card
    ...
    <div className="w-full min-h-screen bg-background">
      ...
      >
        <div className="w-full max-w-6xl mx-auto">
            ...
            {teamMembers.map((member) => (
              ...
              >
                <div className="w-full aspect-square bg-gray-200 dark:bg-gray-800 rounded-lg overflow-hidden relative">
                  ...
                  <div className={`absolute inset-0 bg-white/95 dark:bg-gray-900/95 transition-opacity duration-300 flex flex-col p-2 md:p-4 overflow-y-auto ${
                    isMobile 
                    ...
                    <div className="flex justify-center items-center gap-2 md:gap-6 mb-2 md:mb-4 flex-1">
                      {member.companies.map((company, index) => (
                        <img 
                          ...
                          src={company.logo} 
                          alt={company.name} 
                          className="h-24 md:h-32 w-auto opacity-80 invert dark:invert-0" 
                        />
                      ))}
                    </div>
                    ...
                    <div className="mt-auto">
                      <div className="flex items-center gap-1 md:gap-2 mb-1">
                        <p className="text-xl md:text-2xl tracking-tighter leading-tight text-gray-900 dark:text-gray-100">{member.name}</p>
                        <LinkedInIcon size={12} />
                      </div>
                      <p className="text-sm uppercase tracking-wide text-blue-600 dark:text-blue-400 mb-1 md:mb-2">{member.title}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-1 md:mb-2">{member.description}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          {/* Footer - Centered Shield (Same as Manifesto Page) */}
